...
class TransactionService:
    ...
    async def get_transaction_details(
        ...
        if user_id > 0:
            result = await self.session.execute(
                select(Alerts).filter(
                    Alerts.transaction_hash == tr_hash, Alerts.tg_id == user_id
                )
            )
        ...
        try:
            ...
            # Handle bad JSON
            return {"error": "BAD xdr. Can`t load", "transaction": transaction}
        ...
        for address in json_transaction:
            ...
            sorted_signers = sorted(
                json_transaction[address]["signers"], key=lambda x: x[1]
            )
            ...
            for signer in sorted_signers:
                ...
                if signature:
                    if (
                        has_votes < int(json_transaction[address]["threshold"])
                        or int(json_transaction[address]["threshold"]) == 0
                    ):
        ...
        db_signatures = await self.repo.get_all_signatures_for_transaction(
            transaction.hash
        )
        ...
        if signer_ids:
            result = await self.session.execute(
                select(Signers).filter(Signers.id.in_(signer_ids))
            )
    ...
    async def update_signature_visibility(self, signature_id: int, hide: bool):
        result = await self.session.execute(
            select(Signatures).filter(Signatures.id == signature_id)
        )
    ...
    async def sign_transaction_from_xdr(self, tx_body_xdr: str) -> Dict[str, Any]:
        ...
        try:
            tr_full = TransactionEnvelope.from_xdr(
                tx_body_xdr, network_passphrase=Network.PUBLIC_NETWORK_PASSPHRASE
            )
        ...
        if len(tr_full.signatures) > 0:
            ...
            user_map = await load_users_from_grist(
                [s.public_key for s in all_db_signers]
            )
            ...
            for signature in tr_full.signatures:
                ...
                if existing_sig_res.scalars().first():
                    result["MESSAGES"].append(
                        f"Can`t add {username if db_signer else None}. Already was added."
                    )
                else:
                    ...
                    json_signer = list(
                        filter(
                            lambda x: x[2] == signature.signature_hint.hex(), all_sign
                        )
                    )

                    if len(json_signer) == 0:
                        result["MESSAGES"].append(
                            f"Bad signature. {signature.signature_hint.hex()} not found"
                        )
                    else:
                        user_keypair = Keypair.from_public_key(json_signer[0][0])
                        try:
                            user_keypair.verify(
                                data=tr_full.hash(), signature=signature.signature
                            )
                            ...
                            result["MESSAGES"].append(
                                f"Bad signature. {signature.signature_hint.hex()} not verify"
                            )
    ...
    async def alert_signers_notify(
        self, tr_hash: str, small_text: str, tx_description: str
    ):
        text = (
            f'Transaction <a href="https://eurmtl.me/sign_tools/{tr_hash}">{tx_description}</a> : '
            f"{small_text}."
        )
        result = await self.session.execute(
            select(Alerts).filter(Alerts.transaction_hash == tr_hash)
        )
    ...
    @async_cache_with_ttl(
        ttl_seconds=7 * 24 * 60 * 60, maxsize=30
    )  # Кеш на неделю с лимитом 30 транзакций
    async def create_transaction_uri(self, tr_hash: str) -> Optional[str]:
        ...
        transaction_envelope = TransactionEnvelope.from_xdr(
            transaction.body, Network.PUBLIC_NETWORK_PASSPHRASE
        )
    ...
    async def add_or_remove_alert(self, tr_hash: str, tg_id: int) -> Dict[str, Any]:
        try:
            result = await self.session.execute(
                select(Alerts).filter(
                    Alerts.transaction_hash == tr_hash, Alerts.tg_id == tg_id
                )
            )
            alert = result.scalars().first()
            if alert is None:
    ...
    async def get_pending_transactions_for_signer(
        self, public_key: str
    ) -> List[Dict[str, Any]]:
