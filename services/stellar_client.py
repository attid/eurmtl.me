...
from stellar_sdk import (
    Keypair, ServerAsync, AiohttpClient, Asset, TransactionBuilder, Network, TransactionEnvelope,
    LiquidityPoolAsset, TrustLineFlags
)
...
from other.grist_tools import get_secretaries, load_users_from_grist, load_user_from_grist
...
def get_operation_threshold_level(operation) -> str:
    ...
    MEDIUM_THRESHOLD_OPS = {
        ...
        'SetOptions', 'ChangeTrust', 'ManageData', 'CreateClaimableBalance',
    ...
    LOW_THRESHOLD_OPS = {
        'BumpSequence', 'AllowTrust', 'SetTrustLineFlags', 'ClaimClaimableBalance'
    }
...
async def stellar_build_xdr(data):
    ...
    for operation in data['operations']:
        ...
        if operation['type'] == 'pay_divs':
            ...
            raw_flag = operation.get('require_trustline') if 'require_trustline' in operation else operation.get(
                'requireTrustline')
