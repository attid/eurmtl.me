    ...
    if tg_web_app_start_param:
        ...
        async with current_app.db_pool() as db_session:
            if chat_id == 0:
                result = await db_session.execute(
                    select(WebEditorMessages).filter(
        ...
        if message_record:
            ...
            return await render_template(
                "tabler_web_editor.html", message_text=message_record.message_text
            )
        else:
            if chat_id == 0:
                abort(404)
            ...
            try:
                ...
                return await render_template(
                    "tabler_web_editor.html", message_text="Your text here..."
                )
...
async def get_reply_markup_aiogram(
        ...
        async with current_app.db_pool() as db_session:
            ...
            if message_record:
                ...
                new_message_record = WebEditorMessages(
                    chat_id=chat_id, message_id=message_id, message_text=data["text"]
                )
            ...
            try:
                ...
                return jsonify(
                    {"ok": False, "error": "Ошибка при редактировании сообщения"}
                ), 500
...
async def join_captcha():
    ...
    if request.method == "POST":
        ...
        )  # Changed from request.json to request.get_json()
        init_data_str = data.get("initData")
