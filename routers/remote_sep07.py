...
from stellar_sdk import Network, TransactionEnvelope, TransactionBuilder
        ...
        return cors_jsonify(
            {
                "status": "failed",
                "hash": "",
                "error": {"message": "Invalid or missing base64 data", "details": None},
            },
            400,
        )  # Bad Request
...
async def remote_add_uri():
    ...
    try:
        ...
        try:
            transaction_uri = stellar_uri.TransactionStellarUri.from_uri(
                uri, network_passphrase=Network.PUBLIC_NETWORK_PASSPHRASE
            )
            ...
            try:
                transaction_uri = stellar_uri.TransactionStellarUri.from_uri(
                    decoded_uri, network_passphrase=Network.PUBLIC_NETWORK_PASSPHRASE
                )
                logger.debug(
                    f"Successfully parsed URI after decoding: {transaction_uri}"
                )
            except Exception as e:
                logger.error(f"Failed to parse URI after decoding: {str(e)}")
                return cors_jsonify(
                    {"SUCCESS": False, "message": f"Invalid URI: {str(e)}"}, 400
                )
        ...
        try:
            ...
            async with current_app.db_pool() as db_session:
                result = await db_session.execute(
                    select(MMWBTransactions).filter_by(uuid=key)
                )
                ...
                if existing_transaction:
                    ...
                    transaction = MMWBTransactions(
                        uuid=key, json=json.dumps({"uri": uri})
                    )
            ...
            if status == "exists_different":
                logger.error(
                    "Transaction with same UUID but different URI already exists"
                )
                return cors_jsonify(
                    {
                        "SUCCESS": False,
            ...
            return cors_jsonify(
                {"SUCCESS": False, "message": f"Database error: {str(e)}"}, 500
            )
        ...
        return cors_jsonify({"SUCCESS": True, "url": url}, 200)
    except Exception as e:
...
async def get(uuid_key):
    ...
    try:
        async with current_app.db_pool() as db_session:
            result = await db_session.execute(
                select(MMWBTransactions.json).filter(MMWBTransactions.uuid == uuid_key)
            )
            transaction_json = result.scalars().first()
        if not transaction_json:
            return cors_jsonify({"SUCCESS": False, "message": "URI not found"}, 404)
        ...
        if not uri:
            return cors_jsonify(
                {"SUCCESS": False, "message": "URI data is corrupted"}, 500
            )

        return cors_jsonify({"SUCCESS": True, "uri": uri}, 200)
    except Exception as e:
        return cors_jsonify(
            {"SUCCESS": False, "message": f"Error retrieving URI: {str(e)}"}, 500
        )
...
async def parse_uri():
    ...
    try:
        ...
        if not uri:
            return cors_jsonify({"SUCCESS": False, "message": "URI is required"}, 400)
        ...
        if not xdr_param:
            return cors_jsonify(
                {"SUCCESS": False, "message": "No XDR found in URI"}, 400
            )
        ...
        try:
            transaction_envelope = TransactionEnvelope.from_xdr(
                decoded_xdr, Network.PUBLIC_NETWORK_PASSPHRASE
            )
            transaction = transaction_envelope.transaction
        except Exception as e:
            logger.error(f"Failed to parse XDR: {str(e)}")
            return cors_jsonify(
                {"SUCCESS": False, "message": f"Invalid XDR format: {str(e)}"}, 400
            )
        ...
        if not callback_url:
            return cors_jsonify(
                {"SUCCESS": False, "message": "No callback URL found in URI"}, 400
            )
        ...
        try:
            ...
            source_account = Account(
                account, -1
            )  # -1 means we don't know the sequence number
        ...
        return cors_jsonify(
            {"SUCCESS": False, "message": f"Error parsing URI: {str(e)}"}, 500
        )
...
async def submit_signed():
    ...
    try:
        ...
        if not is_valid_base64(signed_xdr):
            return cors_jsonify(
                {"SUCCESS": False, "message": "Invalid signed XDR format"}, 400
            )
        ...
        return cors_jsonify(
            {"SUCCESS": False, "message": f"Error submitting signed XDR: {str(e)}"}, 500
        )
...
if __name__ == "__main__":
    print(
        asyncio.run(print("GDLTH4KKMA4R2JGKA7XKI5DLHJBUT42D5RHVK6SS6YHZZLHVLCWJAYXI"))
    )
