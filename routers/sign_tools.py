...
_IPFS_MANAGE_DATA_RE = re.compile(
    r"ManageData\s+(ipfshash(?:-[A-Za-z0-9_.-]+)?)\s*=\s*(.+)$"
)
...
def _extract_ipfs_cid_from_manage_data_line(line: str) -> str | None:
    ...
    match = _IPFS_MANAGE_DATA_RE.search(line)
    if not match:
        return None
...
def _append_ipfs_preview_link(line: str) -> str:
    cid = _extract_ipfs_cid_from_manage_data_line(line)
    if not cid:
        return line

    local_url = f"/ipfs/{quote(cid, safe='')}"
    return f'{line} | <a href="{local_url}" target="_blank">View IPFS metadata</a>'


def _decode_fulldescription(metadata: dict) -> str | None:
    for key in ("fulldescription", "fullDescription", "full_description"):
...
def _render_fulldescription_pretty(text: str | None) -> str | None:
    if not text:
        return None
    ...
    def flush_paragraph() -> None:
        ...
        if paragraph_lines:
            joined = "<br>".join(paragraph_lines)
            output.append(f"<p>{_format_inline_markup(joined)}</p>")
            paragraph_lines = []
    ...
    for line in lines:
        ...
        if heading_match:
            ...
            title = _format_inline_markup(heading_match.group(2))
        ...
        if list_match:
            ...
            if not in_list:
                output.append("<ul>")
                in_list = True
            output.append(f"<li>{_format_inline_markup(list_match.group(1))}</li>")
            continue
...
def _build_metadata_preview(metadata: dict | None) -> dict | None:
    if not metadata:
        return metadata
    ...
    for key, value in metadata.items():
        if key in (
            ...
            if len(value) > 140:
                preview[key] = f"{value[:140]}... (hidden, {len(value)} chars)"
            else:
                preview[key] = value
            continue

        if isinstance(value, str) and len(value) > 240:
            preview[key] = f"{value[:240]}... (truncated, {len(value)} chars)"
            continue
...
async def _fetch_ipfs_metadata(cid: str) -> tuple[dict | None, str | None, str | None]:
    last_error = None
    ...
    for gateway in IPFS_GATEWAYS:
        ...
        try:
            response = await http_session_manager.get_web_request(
                "GET", url=url, return_type="text"
            )
            if response.status != 200:
                last_error = f"HTTP {response.status} from {url}"
                continue

            payload = response.data
            metadata = payload if isinstance(payload, dict) else json.loads(payload)
            if isinstance(metadata, dict):
                return metadata, url, None

            last_error = f"Non-JSON object from {url}"
...
async def start_add_transaction():
    ...
    return await render_template(
        "tabler_sign_add.html", xdr=xdr, description=description, memo=memo
    )
...
async def show_transaction(tr_hash):
    ...
    async with current_app.db_pool() as db_session:
        ...
        if "error" in data:
            ...
            return await render_template(
                "tabler_sign_add.html", tx_description="", tx_body=""
            )
    ...
    if send is not None:
        ...
        try:
            ...
            if transaction_resp.status == 200:
                ...
                if tx_hash:
                    msg += Markup(
                        f' <a href="https://viewer.eurmtl.me/transaction/{tx_hash}" target="_blank">View in Explorer</a>'
                    )
                await flash(msg, "good")
            else:
                if isinstance(transaction_resp.data, dict):
                    await flash(
                        f"Failed to send. {transaction_resp.data.get('extras', {}).get('result_codes')}"
                    )
                    result_codes = transaction_resp.data.get("extras", {}).get(
                        "result_codes", {}
                    )
                    ...
                    for i, result in enumerate(operation_results):
                        if result != "op_success":
                            ...
                            failed_operation_dict = "<br>".join(
                                await decode_xdr_to_text(
                                    transaction.body, only_op_number=i
                                )
                            )
                            await flash(
                                Markup(
                                    f"Details of failed operation: {failed_operation_dict}"
                                )
                            )
                            break
                else:
                    ...
                    await flash(
                        "Failed to send. Received unexpected response format from Horizon."
                    )
...
async def start_show_all_transactions():
    ...
    my_transactions = request.args.get(
        "my_transactions", default=False, type=lambda v: v.lower() == "on"
    )
...
async def decode_xdr(tr_hash):
    ...
    encoded_xdr = [_append_ipfs_preview_link(line) for line in encoded_xdr]
    return (
        ("<br>".join(encoded_xdr) + "<br><br><br>")
        .replace("\n", "<br>")
        .replace("  ", "&nbsp;&nbsp;")
    )


@blueprint.route("/ipfs/<cid>", methods=("GET",))
async def ipfs_preview(cid):
    if not re.fullmatch(r"[A-Za-z0-9]+", cid):
        abort(404)

    metadata, fetched_from, error_message = await _fetch_ipfs_metadata(cid)
    decoded_fulldescription = (
        _decode_fulldescription(metadata or {}) if metadata else None
    )
    pretty_fulldescription = _render_fulldescription_pretty(decoded_fulldescription)
    metadata_preview = _build_metadata_preview(metadata)
    ...
    return await render_template(
        ...
        decoded_fulldescription=decoded_fulldescription,
        pretty_fulldescription=pretty_fulldescription,
    )
...
async def generate_transaction_qr(tr_hash):
    ...
    if os.path.exists(
        full_path + "88"
    ):  # check original code had + '88'? Wait, line 511: if os.path.exists(full_path+'88'):
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": f"Error: {str(e)}", "file": "", "uri": ""}
        )
