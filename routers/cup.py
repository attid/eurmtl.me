...
async def cmd_orderbook(asset1, asset2):
    ...
    orders = {"sellers": [], "buyers": []}
    need_round = 7
    try:
        server = Server(horizon_url="https://horizon.stellar.org")

        sellers_offers = (
            server.offers().for_selling(asset1).for_buying(asset2).limit(200).call()
        )
        buyers_offers = (
            server.offers().for_buying(asset1).for_selling(asset2).limit(200).call()
        )

        sellers_offers["_embedded"]["records"] = sorted(
            sellers_offers["_embedded"]["records"],
            key=lambda x: float(x["price"]),
            reverse=True,
        )
        buyers_offers["_embedded"]["records"] = sorted(
            buyers_offers["_embedded"]["records"],
            key=lambda x: float(x["price"]),
            reverse=False,
        )
        ...
        for offer in sellers_offers["_embedded"]["records"]:
            ...
            if existing_order:
                ...
                existing_order["total"] += round(
                    float(offer["amount"]) * price_rounded, need_round
                )
            else:
        ...
        orders["sellers"] = sorted(
            orders["sellers"], key=lambda x: x["price"], reverse=True
        )
        ...
        orders["buyers"] = sorted(
            orders["buyers"], key=lambda x: x["price"], reverse=True
        )
    ...
    lowest_sell_price = (
        min(order["price"] for order in orders["sellers"]) if orders["sellers"] else 1
    )
    highest_buy_price = (
        max(order["price"] for order in orders["buyers"]) if orders["buyers"] else 1
    )
    ...
    asset_url = (
        f"{asset_url}/{asset2.code}-{asset2.issuer}"
        if asset2.issuer
        else f"{asset_url}/{asset2.code}"
    )
    resp = await render_template(
        ...
        asset_url=asset_url,
        scopuly_url=get_scopuly_url(asset1, asset2),
...
async def cmd_chart(asset1, asset2):
    ...
    try:
        ...
        for offer in sellers_offers["_embedded"]["records"]:
            ...
            if existing_order:
                ...
                existing_order["total"] += round(
                    float(offer["amount"]) * price_rounded, 3
                )
            else:
                # Иначе добавляем новый заказ
                order_info = {
    ...
    max_sellers_amount = (
        max(orders["sellers"], key=lambda x: x["amount"])["amount"]
        if orders["sellers"]
        else 0
    )
    max_buyers_amount = (
        max(orders["buyers"], key=lambda x: x["amount"])["amount"]
        if orders["buyers"]
        else 0
    )
    ...
    asset_url = f"{asset1.code}-{asset1.issuer}/{asset2.code}-{asset2.issuer}"
    resp = await render_template(
        "cup.chart.html", orders=orders, assets=[asset1, asset2], asset_url=asset_url
    )
    return resp
