...
async def grist_tg_info_groups(user_id):
    ...
    key_check = await check_grist_key(
        auth_key, log_info=f"grist_tg_info_groups {user_id}"
    )
            ...
            chat_data.append(
                {
                    ...
                    "joined": user_data.created_at.strftime("%Y-%m-%d %H:%M")
                    if user_data.created_at
                    else "-",
                    "left": user_data.left_at.strftime("%Y-%m-%d %H:%M")
                    if user_data.left_at
                    else "-",
                }
            )

        return await render_template("grist_tg_groups.html", chats=chat_data)

    except Exception as e:
        logger.error(f"Error getting chat info: {e}")
        return jsonify(
            {"status": "error", "message": "Ошибка при получении данных о чатах"}
        ), 500
...
async def _process_grist_key(key: str, record: dict):
    ...
    try:
        ...
        logger.info(
            f"Grist webhook: запись id={rec_id} для ключа '{key}' успешно обработана."
        )

    except Exception as e:
        logger.error(
            f"Grist webhook: ошибка при обработке ключа '{key}' для id={rec_id}: {e}"
        )
...
async def check_grist_key(key: str, log_info: str = None) -> dict:
    ...
    try:
        ...
        return {"status": "error", "message": "Ошибка при проверке ключа"}
...
async def grist_tg_info_menu():
    ...
    menu_data = {
        "buttons": [
            {"title": "Группы", "endpoint": "https://eurmtl.me/grist/groups/$user_id$"},
            {
...
async def grist_sky_test(user_id: str):
    ...
    auth_key = request.headers.get("X-Auth-Key")
    key_check = await check_grist_key(auth_key, log_info=f"grist_sky_test {user_id}")
    if key_check["status"] != "success":
        return jsonify(key_check), 403
    ...
    if not user_id:
        return jsonify(
            {"status": "error", "message": "Необходимо указать user_id"}
        ), 400
    ...
    try:
        ...
        # Проверяем подписку
        try:
            await skynet_bot.get_chat(user_id_int)
            return """
        ...
        return jsonify(
            {"status": "error", "message": "Некорректный формат user_id"}
        ), 400
    except Exception as e:
        logger.error(f"SkyNet test error: {e}")
        return jsonify(
            {"status": "error", "message": "Ошибка при проверке подписки"}
        ), 500
...
async def grist_webhook_table(table_name: str):
    ...
    auth_header = request.headers.get("X-Auth-Key") or request.headers.get(
        "Authorization"
    )
    auth_key = None
    if auth_header:
