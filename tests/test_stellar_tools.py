import pytest
from stellar_sdk import Keypair, Signer, SignerKey, Asset, TrustLineEntryFlag
...
@pytest.mark.parametrize(
    ...
    [
        ...
        SetOptions(
            signer=Signer(
                SignerKey.from_encoded_signer_key(Keypair.random().public_key), 1
            )
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        SetOptions(
            home_domain="eurmtl.me"
        ),  # SetOptions без изменения подписчиков/порогов
        PathPaymentStrictReceive(
            destination=Keypair.random().public_key,
...
@pytest.mark.parametrize(
    ...
    [
        SetTrustLineFlags(
            trustor=Keypair.random().public_key,
            asset=Asset("USD", Keypair.random().public_key),
            set_flags=TrustLineEntryFlag.AUTHORIZED_FLAG,
        ),
