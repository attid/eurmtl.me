...
from other.web_tools import (
    http_session_manager,
    WebResponse,
)  # Assuming http_session_manager is in web_tools

PACKAGES_URL = (
    "https://pkgs.tailscale.com/stable/ubuntu/dists/focal/main/binary-amd64/Packages"
)
...
async def get_latest_version_package() -> str:
    ...
    packages_response: WebResponse = await http_session_manager.get_web_request(
        "GET", PACKAGES_URL, return_type="text"
    )
    if packages_response.status != 200:
        raise RuntimeError(
            f"Failed to fetch packages list: {packages_response.status} {packages_response.data}"
        )

    content = packages_response.data
    if not isinstance(content, str):  # Should be text
        raise RuntimeError(
            f"Unexpected content type for packages list: {type(content)}"
        )
    ...
    deb_response: WebResponse = await http_session_manager.get_web_request(
        "GET", download_url, return_type="bytes"
    )

    if deb_response.status != 200:
        raise RuntimeError(
            f"Failed to download package: {deb_response.status} {deb_response.data}"
        )
