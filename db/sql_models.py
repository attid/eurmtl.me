...
class Transactions(Base):
    ...
    )  # 0-new 1-need_sent 2-was_send 3-cancel
...
class Signatures(Base):
    ...
    transaction_hash = Column(
        "transaction_hash", String(64), ForeignKey("t_transactions.hash")
    )
...
class MMWBTransactions(Base):
    ...
    uuid = Column(
        "uuid", String(32), primary_key=True, default=lambda: uuid.uuid4().hex
    )
    tg_id = Column("tg_id", BigInteger(), nullable=True)
    json = Column("json", Text(), nullable=True)
    dt = Column(DateTime(), default=datetime.now)
...
class Sep6Transactions(Base):
    ...
    uuid = Column(
        "uuid", String(32), default=lambda: uuid.uuid4().hex, primary_key=True
    )
    ...
    external_transaction_id = Column(
        "external_transaction_id", String(64), nullable=True
    )
